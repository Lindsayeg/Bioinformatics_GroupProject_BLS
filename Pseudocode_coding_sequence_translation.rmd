...
```
This is a plan for taking a fasta nucleotide sequence (DNA) file as input and searching for a protein-coding strand. Note that this code could also be made into a script, with parameters to replace file names and such.
...
Note: Perhaps the simplest way to do this would be to make the sequence subsettable, and the use regex to find all nucleotide patterns with "ATG[ATGC]*TAA" and repeated for the other stop codons. Note that by altering the "value" option in the grep function one can ask for the indix location of the matches within the dataframe or the matched themselves. One could check if the index for the first letter is either 1 or a multiple of 4 (need to find a function for this) to see if it corresponds to the beigining of an in-frame codon. The same would be done to see if the whole match is divisible by 3 to make sure it is a complete codon sequence. 